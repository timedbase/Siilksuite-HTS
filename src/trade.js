...
async function ensureTokenAssociation(network, operatorId, operatorPrivateKeyStr, tokenId) {
  // Skip for HBAR (native token, no association needed)
  if (tokenId.toUpperCase() === 'HBAR') {
    console.log(`💰 No association needed (${tokenId} is native token)`);
    return;
  }

  console.log(`\n🔗 Checking token association for ${tokenId}...`);

  try {
    // Check current association status via Mirror Node
    try {
      const mirrorClient = axios.create({ baseURL: 'https://mainnet-public.mirrornode.hedera.com', timeout: 10000 });
      const acctResp = await mirrorClient.get(`/api/v1/accounts/${operatorId}`);

      if (acctResp && acctResp.data && acctResp.data.balance) {
        // Check if tokenId appears in associated tokens list
        const associatedTokens = acctResp.data.balance.tokens || [];
        const isAssociated = associatedTokens.some((t) => t.token_id === tokenId);

        if (isAssociated) {
          console.log(`✅ Already associated with token ${tokenId}`);
          return;
        }
      }
    } catch (mirrorErr) {
      console.warn('⚠️ Could not check association status from Mirror Node, proceeding with association attempt.');
    }

    // Not associated — create association transaction
    console.log(`📝 Creating token association transaction...`);

    // Set up Hedera client for mainnet
    const client = Client.forMainnet();
    const privKey = PrivateKey.fromString(operatorPrivateKeyStr);
    const opId = AccountId.fromString(operatorId);

    client.setOperator(opId, privKey);
    ...
    if (receipt.status.toString() === 'SUCCESS') {
      console.log(`✅ Token association successful (txId: ${txResponse.transactionId})`);
    } else {
    ...
    throw new Error(`Token association failed: ${err && err.message ? err.message : err}`);
  }
}
